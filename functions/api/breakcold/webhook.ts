/**
 * Breakcold Webhook Endpoint for AutiMind
...
const handleLeadCreated = (data: BreakcoldWebhookPayload['payload']) => {
  console.log('AutiMind lead created:', data);
  
  if (data.email) {
    console.log(`New AutiMind lead captured: ${data.firstName} ${data.lastName} (${data.email}) from ${data.source || 'unknown source'} - Interest: ${data.customAttributes?.interest || 'General'}`);
  }
};

const handleLeadUpdated = (data: BreakcoldWebhookPayload['payload']) => {
  console.log('AutiMind lead updated:', data);
  
  if (data.status === 'Qualified') {
    console.log(`AutiMind lead ${data.id} has been qualified - triggering follow-up sequence`);
  }
};

const handleLeadDeleted = (data: BreakcoldWebhookPayload['payload']) => {
  console.log('AutiMind lead deleted:', data);
  console.log(`Cleaning up data for deleted AutiMind lead: ${data.id}`);
};
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (!verifyWorkspace(payload.id_space, workspaceId)) {
      console.error('AutiMind workspace verification failed:', {
    ...
    console.log('Received AutiMind Breakcold webhook:', {
      event: payload.event,
    ...
    switch (payload.event) {
      ...
      case 'lead.status_changed':
        console.log('AutiMind lead status changed:', {
      ...
      case 'lead.tags_updated':
        console.log('AutiMind lead tags updated:', {
          id: payload.payload.id,
      ...
      default:
        console.log('Unhandled AutiMind webhook event:', payload.event);
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: 'AutiMind webhook processed successfully',
      event: payload.event,
      workspace: payload.id_space,
      leadId: payload.payload.id,
      timestamp: new Date().toISOString()
    ...
    console.error('AutiMind webhook processing error:', error);
